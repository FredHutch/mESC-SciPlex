...
# Export gene lists
list_of_genesets <- list(PS,LPM,IEC,EEC,Mesenchyme,Cardiomyocytes,EXE_mesoderm,AEC1_Kalucka,AEC2_Xu)
names(list_of_genesets) <- c("PS_genelist","LPM_genelist","IEC_genelist","EEC_genelist","Mesenchyme_genelist","Cardiomyocytes_genelist","EXE_mesoderm_genelist","AEC1_Kalucka_genelist","AEC2_Xu_genelist")
#For loop to write gene lists to csv
for (geneset_name in names(list_of_genesets)) {
  # Extract the geneset from the list
  geneset <- list_of_genesets[[geneset_name]]
  # Write gene set list
  write.csv(geneset, file.path(plotdir, filename = paste0(geneset_name, ".csv")))
}
